...
=head1 SYNOPSIS
    ...
    cat <infile.vcf> | perl SNPcleaner.pl [OPTIONS]

  Options:
        -?       this help screen
        -2       keep non-biallelic sites
	-d INT   minimum site read depth
	-D INT   maximum site read depth
	-k INT   min number of individuals with less than -u INT coverage (requires -u and mpileup -D)
	-u INT   minimum individual coverage threshold used in -k (requires -k and mpileup -D)
	-a INT   minimum number of alternate alleles per site
	-Q INT   minimum RMS mapping quality for SNPS
	-S FLOAT min p-value for strand bias
	-b FLOAT min p-value for base quality bias
	-f FLOAT min p-value for map quality bias
	-e FLOAT min p-value for end distance bias
	-h FLOAT min p-value for exact test of HWE
	-H FLOAT min p-value for exact test on excess of heterozygous
	-L FLOAT min p-value for exact test on defect of heterozygous
	-F FLOAT inbreeding coefficient value
	...
	-v	 keep nonvariant sites
	-I FILE	 output file with individual mean depth
	-P FILE  input file with population information (sample_name\tpop_id) for HWE filtering


=head1 DESCRIPTION

    This script will read a VCF file and filter SNPs based on a set of rules. It is based on SAMTOOLS 
    "varfilter.pl" script but extends some of the filters.
    NOTE: This script is only for SNP filtering and will ignore INDELs!

=head1 AUTHOR

    Tyler Lynderoth - tylerp.linderoth _at_ gmail _dot_ com
...
my %opts = ('?'=>0, '2'=>undef, 'd'=>2, 'D'=>1000000, 'k'=>1, 'u'=>0, 'a'=>0, 'Q'=>10, 'S'=>1e-4, 'b'=>1e-100, 'f'=>0, 'e'=>1e-4, 'h'=>0, 'F'=>0, 'H'=>0, 'L'=>0, 'A'=>undef, 'M'=>undef, 'B'=>undef, 'p'=>undef, 'r'=>undef, 'X'=>undef, 't'=>undef, 'g'=>undef, 'v'=>undef, 'I'=>undef, 'P'=>"output");

getopts('?2d:D:k:u:a:Q:S:b:f:e:h:F:H:L:A:M:B:p:r:X:I:Z:P:tgv', \%opts);

die (qq/
#####################
# SNPcleaner v$version #
#####################
Usage: cat <infile.vcf> | SNPcleaner.pl [OPTIONS]
...
Options:
	...
	-a INT   minimum number of alternate alleles per site [$opts{a}]
	...
	-H FLOAT min p-value for exact test on excess of heterozygous [$opts{H}]
	-L FLOAT min p-value for exact test on defect of heterozygous [$opts{L}]
...
Note: Some of the filters rely on annotations generated by SAMtools\/BCFtools.
      To use the even coverage filter with options -k and -u, -D must be used with samtools mpileup.
...
my ($excont, @t, @seq, @snp_buffer, %exons, %ancestral);
my ($n_sites, $ind_dp, $prev_format, @format, @ind_pop, @ind_depth) = (0,0,"");
